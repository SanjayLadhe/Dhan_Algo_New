...
def process_symbol(name, chart, all_ltp):
	...
	try:
		...
		chart['vwap'] = volume_weighted_average_price(high=chart['high'], low=chart['low'], close=chart['close'],
		                                              volume=chart['volume'], window=14, fillna=True)
		...
		print(f" {name} : {bc1}, {bc2}, {bc3}, {bc4}, {bc5} : {process_start_time} \n")

		if bc1 and bc2 and bc3 and bc4 and bc5 and bc6 and bc7:
			print("buy ", name, "\t")

			try:
				atm_result = tsl.ATM_Strike_Selection(Underlying=name, Expiry=0)
				if len(atm_result) >= 4:
					ce_name, pe_name, ce_atm_strike, pe_atm_strike = atm_result
				elif len(atm_result) == 3:
					ce_name, ce_atm_strike, pe_atm_strike = atm_result
					pe_name = None  # or handle appropriately
				else:
					print(f"ATM_Strike_Selection returned unexpected number of values: {len(atm_result)}")
					return name, "ATM_Strike_Selection failed"
			except Exception as e:
				print(f"Error in ATM_Strike_Selection for {name}: {e}")
				return name, f"ATM_Strike_Selection error: {str(e)}"
