...
def check_target_hit(name, orderbook, all_ltp, target_percentage=None):
    ...
    Args:
        name: Stock symbol
        orderbook: Order tracking dictionary
        all_ltp: Dictionary of current prices
    ...
    try:
        ...
        if target_percentage is None:
            # If risk_reward is 3, target is 3x the risk
            risk_reward = 3  # This should be passed as parameter
            target_price = entry_price + (risk_per_unit * risk_reward)
        else:
            target_price = entry_price * (1 + target_percentage / 100)
        
        # Get current LTP
        ltp = all_ltp.get(name, 0)
        
        # Check if target reached
        target_hit = ltp >= target_price
        
        if target_hit:
            print(f"🎯 Target hit for {name}! Entry: {entry_price}, Target: {target_price}, LTP: {ltp}")
...
def handle_time_exit(tsl, name, orderbook, process_start_time, all_ltp,
    ...
    try:
        ...
        if current_pnl >= 0:
            print(f"Time exceeded for {name} but position is profitable. Not exiting.")
            return False
...
def update_trailing_stop_loss(tsl, name, orderbook, atr_multipler):
    ...
    try:
        ...
        # Calculate ATR
        options_chart_tsl['atr'] = talib.ATR(
            options_chart_tsl['high'],
            options_chart_tsl['low'],
            options_chart_tsl['close'],
            timeperiod=14
        )
...
def check_rsi_longstop_exit(tsl, name, orderbook):
    ...
    try:
        ...
        # Fetch option data
        data_api_limiter.wait(
            call_description=f"tsl.get_historical_data(tradingsymbol='{options_name}', exchange='NFO', timeframe='3')"
        )
        ...
        # Calculate RSI and MA-RSI
        options_chart['rsi'] = talib.RSI(options_chart['close'], timeperiod=14)
        options_chart['ma_rsi'] = talib.SMA(options_chart['rsi'], timeperiod=20)
        
        # Calculate Long_Stop using ATR
        atr_strategy = ATRTrailingStopIndicator(period=21, multiplier=1.0)
        result_df = atr_strategy.compute_indicator(options_chart)
        options_chart['Long_Stop'] = result_df['Long_Stop']
        
        # Get latest candle
        latest = options_chart.iloc[-1]
        previous = options_chart.iloc[-2]
