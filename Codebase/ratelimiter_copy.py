...
from sector_performance import get_sector_watchlist
    ...
    try:
        ...
        if bc1 and bc2 and bc3 and bc4 and bc5 and bc6 and bc7:
            ...
            if bc1_opt and bc2_opt and bc3_opt and bc4_opt and bc5_opt and bc6_opt and bc7_opt:
                ...
                try:
                    # ⬇️ RATE LIMITED: Order API
                    order_api_limiter.wait(call_description=f"tsl.order_placement(tradingsymbol='{orderbook[name]['options_name']}', exchange='NFO', quantity={orderbook[name]['qty']}, order_type='MARKET', transaction_type='BUY')")
                    ...
                    # ⬇️ RATE LIMITED: Non Trading API
                    ntrading_api_limiter.wait(call_description=f"tsl.get_executed_price(orderid='{orderbook[name]['entry_orderid']}')")
                    exec_price = retry_api_call(tsl.get_executed_price, retries=1, delay=1.0, orderid=orderbook[name]['entry_orderid'])
                    ...
                    # ⬇️ RATE LIMITED: Order API
                    order_api_limiter.wait(call_description=f"tsl.order_placement(tradingsymbol='{orderbook[name]['options_name']}', exchange='NFO', quantity={orderbook[name]['qty']}, order_type='STOPLIMIT', transaction_type='SELL')")
        ...
        if sc1 and sc2 and sc3 and sc4 and sc5 and sc6 and sc7:
            ...
            if bc1_opt and bc2_opt and bc3_opt and bc4_opt and bc5_opt and bc6_opt and bc7_opt:
                ...
                try:
                    ...
                    # ⬇️ RATE LIMITED: Non Trading API
                    ntrading_api_limiter.wait(call_description=f"tsl.get_executed_price(orderid='{orderbook[name]['entry_orderid']}')")
                    exec_price = retry_api_call(tsl.get_executed_price, retries=3, delay=1.0, orderid=orderbook[name]['entry_orderid'])
                    if exec_price is None:
                        raise Exception("Failed to get executed price after retries")
                    orderbook[name]['entry_price'] = exec_price
                    ...
                    message = "\n".join(f"'{key}': {repr(value)}" for key, value in orderbook[name].items())
                    message = f"Entry_done {name} (PE Buy/Sell Signal) \n\n {message}"
                    tsl.send_telegram_alert(message=message, receiver_chat_id=receiver_chat_id, bot_token=bot_token)

                except Exception as e:
                    print(f"Error in entry order for {name} (PE): {e}")
                    traceback.print_exc()
        ...
        if orderbook[name]['traded'] == "yes":
            ...
            if bought:
                try:
                    ltp = all_ltp.get(name, 0) # Use .get to avoid KeyError if name not in all_ltp

                    # ⬇️ RATE LIMITED: Non Trading API
                    ntrading_api_limiter.wait(call_description=f"tsl.get_order_status(orderid='{orderbook[name]['sl_orderid']}')")
                    sl_status = retry_api_call(tsl.get_order_status, retries=1, delay=1.0, orderid=orderbook[name]['sl_orderid'])
                    if sl_status is None:
                         print(f"[WARNING] Failed to get order status for {name}. Assuming UNKNOWN.")
                         sl_status = "UNKNOWN"
                ...
                if sl_hit:
                    try:
                        ...
                        # ⬇️ RATE LIMITED: Non Trading API
                        ntrading_api_limiter.wait(call_description=f"tsl.get_executed_price(orderid='{orderbook[name]['sl_orderid']}')")
                        exit_price = retry_api_call(tsl.get_executed_price, retries=1, delay=1.0, orderid=orderbook[name]['sl_orderid'])
                        if exit_price is None:
                             print(f"[WARNING] Failed to get SL executed price for {name}. Using 0.")
                             exit_price = 0 # Or handle differently
                ...
                if holding_time_exceeded and (current_pnl < 0):
                    try:
                        # ⬇️ RATE LIMITED: Order API
                        order_api_limiter.wait(call_description=f"tsl.cancel_order(OrderID='{orderbook[name]['sl_orderid']}')")
                        tsl.cancel_order(OrderID=orderbook[name]['sl_orderid'])
                        time.sleep(2)
                        # ⬇️ RATE LIMITED: Order API
                        order_api_limiter.wait(call_description=f"tsl.order_placement(tradingsymbol='{orderbook[name]['options_name']}', exchange='NFO', quantity={orderbook[name]['qty']}, order_type='MARKET', transaction_type='SELL')")
                        square_off_buy_order = tsl.order_placement(tradingsymbol=orderbook[name]['options_name'], # Use options_name
                        ...
                        # ⬇️ RATE LIMITED: Non Trading API
                        ntrading_api_limiter.wait(call_description=f"tsl.get_executed_price(orderid='{square_off_buy_order}')")
                        exit_price = retry_api_call(tsl.get_executed_price, retries=1, delay=1.0, orderid=square_off_buy_order)
                        if exit_price is None:
                             print(f"[WARNING] Failed to get square-off executed price for {name}. Using 0.")
                             exit_price = 0 # Or handle differently
                        orderbook[name]['exit_price'] = exit_price
                ...
                try:
                    ...
                    # ⬇️ RATE LIMITED: Non Trading API
                    #ntrading_api_limiter.wait(call_description=f"tsl.get_ltp_data(names=['{options_name}'])")
                    ltp_api_limiter.wait(call_description=f"tsl.get_ltp_data(names=['{options_name}'])")
                    # Fetch LTP only for the specific option being updated
                    options_ltp_data = retry_api_call(tsl.get_ltp_data, retries=1, delay=1.0, names=[options_name])
                    ...
                    if tsl_level > orderbook[name]['tsl']:
                        ...
                        if tsl_qty <= 0:
                             print(f"[WARNING] Invalid quantity {tsl_qty} for TSL update for {name}. Skipping.")
                             return name, "skip_tsl_invalid_qty"

                        # ⬇️ RATE LIMITED: Order API
                        order_api_limiter.wait(call_description=f"tsl.modify_order(order_id='{orderbook[name]['sl_orderid']}', order_type='STOPLIMIT', quantity={tsl_qty})")
                        tsl.modify_order(order_id=orderbook[name]['sl_orderid'], order_type="STOPLIMIT",
                                         quantity=tsl_qty, # ✅ Use correct quantity from orderbook
                                         price=price, trigger_price=trigger_price)
                        orderbook[name]['tsl'] = tsl_level
                        print(f"[TSL] Updated TSL for {name} ({options_name}) to {tsl_level}")
                except Exception as e:
...
while True:
    ...
    if max_loss_hit or market_over:
        # Cancel all orders on exit condition
        try:
            # This might involve multiple order API calls, but we don't have a specific limiter for it.
            # It's usually a one-time call at the end, so we'll allow it without explicit limiting for now.
            # If it causes issues, wrap internal calls with order_api_limiter.
    ...
    # ⬇️ RATE LIMITED: Non Trading API
    # --- Optimized LTP Fetch (Concept) ---
    # Fetch LTP only for symbols currently being traded to reduce API load
    # active_symbols = [name for name, order in orderbook.items() if order.get('traded') == 'yes']
    # all_symbols_to_fetch = list(set(watchlist + active_symbols)) # Combine watchlist (for new entries) and active
    # ntrading_api_limiter.wait(call_description="tsl.get_ltp_data(names=[...])") # Describe the call
    # all_ltp_raw = retry_api_call(tsl.get_ltp_data, retries=3, delay=1.0, names=all_symbols_to_fetch)
    # --- End Optimized LTP Fetch ---

    # --- Original LTP Fetch (Fetches for entire watchlist) ---
    #ntrading_api_limiter.wait(call_description="tsl.get_ltp_data(names=watchlist)")
